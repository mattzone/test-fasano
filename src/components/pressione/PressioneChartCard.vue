<!-- 
  PressioneChartCard
  In questa pagine viene mostrata tramite un grafico la pressione atmosferica delle province italiane.
  ...
  methods: {
    async onSearch() {
      ...
      let promises = provinceSelected.map(async provincia => {
        ...
        return {
          label: provincia,
          umidity: provinciaWeatherData.main.pressure, // Restituisce la pressione
        };
      });
